             ...
             <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                   @for (asset of filteredAssets(); track asset.id) {
                      <tr class="transition-colors group cursor-pointer border-l-4" 
                         ...
                         <td class="p-4">
                            <div class="flex items-center gap-2">
                                <span [class]="'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border ' + asset.status.color">
                                <span class="w-1.5 h-1.5 rounded-full bg-current mr-1.5"></span>
                                {{ asset.status.name }}
                                </span>
                                @if (isCriticalMaintenance(asset)) {
                                    <i class="fas fa-exclamation-triangle text-red-500 text-sm animate-bounce" title="Atención requerida: Tiempo excedido"></i>
                                }
                            </div>
