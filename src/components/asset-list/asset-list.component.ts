    ...
    <div class="space-y-6 relative">
      ...
      >
        ...
        <div class="flex gap-3 w-full md:w-auto">
          ...
          <button
            (click)="generateExecutiveReport()"
            class="px-4 py-2 bg-[#ce1126] hover:bg-[#a30d1d] text-white text-xs font-bold uppercase rounded-lg shadow-sm flex items-center gap-2 transition-transform hover:-translate-y-0.5 active:translate-y-0"
          >
          ...
          <table class="w-full text-sm text-left">
            ...
            <tbody class="divide-y divide-slate-100">
              @for (asset of filteredAssets(); track asset.id) {
                <tr
                  class="transition-colors group cursor-pointer border-l-4 relative"
                ...
                >
                  <td class="p-4 font-black text-slate-800">
                    ...
                    @if (isCriticalMaintenance(asset)) {
                      <div class="mt-1 inline-flex items-center gap-1 text-[9px] uppercase font-bold text-red-600 animate-pulse">
      ...
      @if (hoveredAsset()) {
        <div
          class="fixed z-[9999] bg-slate-900/95 backdrop-blur-sm text-white p-3 rounded-lg shadow-2xl pointer-events-none text-xs border border-slate-700 min-w-[200px]"
          [style.left.px]="tooltipPos().x"
          [style.top.px]="tooltipPos().y"
        >
          <div class="border-b border-slate-700 pb-2 mb-2">
            <div class="font-bold text-white text-sm">
              {{ hoveredAsset()?.brand }} {{ hoveredAsset()?.model }}
            </div>
