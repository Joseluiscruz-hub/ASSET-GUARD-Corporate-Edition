...
import { Component, ElementRef, ViewChild, effect, inject, computed, signal } from '@angular/core';
...
@Component({
    ...
    <div class="min-h-full transition-colors duration-500 ease-in-out font-sans relative"
      ...
      <div class="flex justify-between items-center mb-6">
         <div class="flex items-center gap-4">
            <h2 class="text-2xl font-black uppercase tracking-widest flex items-center gap-2"
                [class.text-[#ce1126]]="plantMode()" [class.text-slate-800]="!plantMode()">
         ...
         <div class="text-xs font-bold font-mono opacity-60" [class.text-white]="plantMode()" [class.text-slate-500]="!plantMode()">
      ...
      <div class="bento-grid">
        ...
        <div class="col-span-12 lg:col-span-6 card-kpi priority-1 group"
            ...
            <div>
                <h3 class="text-xs font-bold uppercase tracking-widest text-emerald-500 mb-1 flex items-center gap-2">
                    <i class="fas fa-hard-hat"></i> Seguridad Industrial
                </h3>
                <div class="flex items-baseline gap-2 mt-2">
                    <div class="text-5xl font-black tracking-tighter" [class.text-white]="plantMode()" [class.text-emerald-900]="!plantMode()">
            ...
            <div class="w-full bg-gray-200/20 h-2 mt-6 rounded-full overflow-hidden border border-emerald-500/20">
                <div class="bg-emerald-500 h-full shadow-[0_0_10px_rgba(16,185,129,0.5)]" [style.width.%]="(safetyStats().daysWithoutAccident / safetyStats().record) * 100"></div>
            </div>
        ...
        <div class="col-span-12 md:col-span-6 lg:col-span-3 card-kpi priority-2 group"
             [class.text-white]="plantMode()" [class.text-slate-800]="!plantMode()">
            <div class="flex justify-between items-start mb-4">
                ...
                <span class="text-[10px] font-bold text-cyan-400 bg-cyan-900/30 px-2 py-0.5 rounded border border-cyan-500/30">KPI</span>
            </div>
            <div class="relative flex items-center justify-center h-32">
                 ...
                 <div class="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
                    ...
                    <span class="text-[8px] uppercase font-bold opacity-50 tracking-wider">Disponibilidad</span>
                 </div>
            </div>
        ...
        <div class="col-span-12 md:col-span-6 lg:col-span-3 card-kpi priority-3 group"
             [class.text-white]="plantMode()" [class.text-slate-800]="!plantMode()">
             <h3 class="text-xs font-bold uppercase tracking-widest text-yellow-400 mb-2 flex items-center gap-2">
                <i class="fas fa-cubes"></i> Volumen
            </h3>
            <div class="mt-2 relative z-10">
                <div class="text-4xl font-black">{{ operativeCount() }} <span class="text-lg opacity-40 font-bold">/ {{ assets().length }}</span></div>
            ...
            <div class="mt-auto pt-4">
                <div class="p-3 bg-yellow-500/10 rounded-lg border border-yellow-500/20 flex justify-between items-center">
                    <div>
        ...
        <div class="col-span-12 card-kpi priority-4 group flex flex-col md:flex-row items-start md:items-center justify-between gap-6"
            ...
            <div class="flex-1">
                ...
                <div class="flex flex-wrap items-baseline gap-4">
                    <span class="text-4xl font-black" [class.text-white]="plantMode()" [class.text-slate-900]="!plantMode()">{{ kpi().totalCostMonth | currency }}</span>
            ...
            <div class="flex-[2] w-full md:border-l pl-0 md:pl-6 border-slate-500/20">
                 ...
                 <div class="relative bg-gradient-to-r from-red-500/10 to-transparent p-3 rounded-lg border-l-2 border-red-500">
        ...
        <div class="col-span-12 lg:col-span-8 card-kpi p-0 relative overflow-hidden flex flex-col min-h-[300px]"
             ...
             <div class="p-6 pb-2">
               <div class="flex justify-between items-center mb-4">
                 ...
                 <span class="text-[10px] font-mono opacity-50 bg-slate-500/20 px-2 py-1 rounded">{{ activeFailures().length }} Tickets</span>
               </div>
             </div>
             ...
             <div class="flex-1 overflow-x-auto p-6 pt-0 custom-scroll snap-x flex gap-4">
                @if (activeFailures().length === 0) {
                   <div class="w-full flex flex-col items-center justify-center opacity-40 border-2 border-dashed rounded-xl border-slate-500/30">
                ...
                @for (f of activeFailures(); track f.id) {
                   <div class="min-w-[280px] max-w-[280px] p-4 rounded-xl border flex flex-col justify-between snap-center transition-all hover:translate-y-[-2px]"
                      ...
                      <div>
                        <div class="flex justify-between items-start mb-2">
                           ...
                           <span class="px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider"
                                 [class.bg-red-500]="f.prioridad === 'Alta'" [class.text-white]="f.prioridad === 'Alta'"
                                 [class.bg-orange-500]="f.prioridad === 'Media'" [class.text-white]="f.prioridad === 'Media'">
                             {{ f.prioridad }}
                           </span>
                        </div>
                      ...
                      <button (click)="closeFailure(f.id)" class="w-full mt-2 bg-green-600/90 hover:bg-green-600 text-white text-[10px] font-bold py-2 rounded uppercase transition-colors">
                            Liberar Unidad
                      </button>
                   </div>
                }
        ...
        <div class="col-span-12 lg:col-span-4 card-kpi p-0 overflow-hidden flex flex-col max-h-[300px]"
             [class.text-white]="plantMode()" [class.text-slate-800]="!plantMode()">
                   ...
                   @for (m of assets(); track m.id) {
                      <div (click)="selectAsset(m)"
                         ...
                         <div class="flex items-center gap-2">
                            @if(m.status.name === 'Taller') { <i class="fas fa-tools text-red-500 text-[10px]"></i> }
        ...
        <div class="col-span-12 card-kpi p-6" [class.text-white]="plantMode()" [class.text-slate-800]="!plantMode()">
             <div class="flex justify-between items-center mb-4">
                <h3 class="text-xs font-bold flex items-center gap-2 uppercase tracking-wide opacity-80">
      ...
      <!-- *** KIOSK MODE OVERLAY *** -->
      @if (isKioskMode()) {
        <div class="fixed inset-0 z-50 bg-[#0a0e14] text-white flex flex-col items-center justify-center p-8 font-sans overflow-hidden cursor-none">
           <!-- Exit Button (Subtle) -->
             ...
             <div class="animate-fade-in flex flex-col items-center justify-center w-full h-full text-center">
                ...
                <div class="flex items-center justify-center gap-12 scale-150 transform">
                   ...
                   <div class="text-left flex flex-col gap-4">
                      ...
                      <div class="text-3xl text-slate-400">{{ operativeCount() }} / {{ assets().length }} Unidades</div>
                   </div>
                </div>
             </div>
           }
                ...
                <div class="grid grid-cols-1 gap-6">
                   ...
                   <div class="bg-gradient-to-r from-orange-600 to-orange-500 p-12 rounded-[3rem] shadow-2xl flex justify-between items-center transform hover:scale-105 transition-transform duration-500">
                      <div class="flex items-center gap-8">
                         ...
                         <div>
                            <h3 class="text-6xl font-black text-white leading-tight">{{ crewStats()[0].name }}</h3>
                            <p class="text-2xl text-orange-100 font-bold uppercase mt-2">Puntuación: {{ crewStats()[0].score }}/100</p>
                         </div>
                      </div>
                   ...
                   <div class="grid grid-cols-2 gap-6">
                      @for (crew of crewStats().slice(1); track crew.rank) {
                        <div class="bg-white/10 p-8 rounded-3xl flex justify-between items-center backdrop-blur-md border border-white/5">
                           <div class="flex items-center gap-4">
                              <span class="text-4xl font-black text-white/40">#{{ crew.rank }}</span>
                              <span class="text-3xl font-bold">{{ crew.name }}</span>
                           </div>
                           <span class="text-4xl font-mono font-bold text-orange-400">{{ crew.pallets }}</span>
                        </div>
                      }
                   </div>
                </div>
           ...
           @if (activeSlide() === 2) {
             <div class="animate-fade-in flex flex-col items-center justify-center w-full h-full">
                ...
                <!-- Announcement Banner -->
                <div class="mt-24 w-full max-w-5xl bg-yellow-500 text-black p-10 rounded-2xl shadow-xl flex items-center gap-8 border-l-[1rem] border-yellow-700 animate-pulse">
           ...
           <!-- Progress Bar -->
           <div class="absolute bottom-0 left-0 h-2 bg-[#ce1126] transition-all duration-[15000ms] ease-linear w-full opacity-50"></div>
        </div>
      }
  ...
  styles: [`
    ...
    /* PRIORITY STRIPS */
    .priority-1 { border-left: 4px solid #10b981; } /* Security - Green */
    .priority-2 { border-left: 4px solid #06b6d4; } /* Quality - Cyan */
    .priority-3 { border-left: 4px solid #eab308; } /* Volume - Yellow */
    .priority-4 { border-left: 4px solid #ef4444; } /* Cost - Red */

    /* SCROLLBARS */
    .custom-scroll::-webkit-scrollbar { height: 6px; width: 6px; }
    .custom-scroll::-webkit-scrollbar-track { background: transparent; }
    .custom-scroll::-webkit-scrollbar-thumb { background: rgba(148, 163, 184, 0.5); border-radius: 3px; }
...
export class DashboardComponent {
  ...
  geminiCostAnalysis = computed(() => {
     ...
     if (ratio < 0.8) return "Gasto controlado. Tendencia positiva de ahorro detectada en mantenimiento correctivo.";
     if (ratio < 1.0) return "En línea con el presupuesto. Se recomienda monitorear refacciones de frenos.";
