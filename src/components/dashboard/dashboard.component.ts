    ...
    <div class="space-y-6 pb-10">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
        ...
        <div class="flex gap-2">
          <button
            ...
            class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 px-4 py-2 rounded-lg text-xs font-bold uppercase hover:bg-slate-50 transition shadow-sm"
          >
            <i class="fas fa-download mr-2"></i> Reporte PDF
          </button>
          <!-- Filter button removed as per feedback to avoid confusion -->
        </div>
      </div>

      <!-- ROW 1: EXECUTIVE KPIS (Refined Logic) -->
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <app-kpi-card
          ...
          unit="días"
          status="success"
          statusText="En buen nivel"
          footerLabel="Meta Histórica"
          [footerValue]="safetyStats().record + ' días'"
          [trendLabel]="
            'Faltan ' + (safetyStats().record - safetyStats().daysWithoutAccident) + ' para récord'
          "
        >
        </app-kpi-card>
        ...
        <app-kpi-card
          ...
          [footerValue]="
            activeFailures().length + (activeFailures().length === 1 ? ' unidad' : ' unidades')
          "
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div
          class="lg:col-span-2 bg-white dark:bg-[#1e293b] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/60 flex flex-col overflow-hidden h-[500px]"
        >
          <div
            class="p-5 border-b border-slate-100 dark:border-slate-700/50 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/20"
          >
            <div>
            ...
            @if (activeFailures().length > 0) {
              <span
                class="text-xs font-bold text-red-600 bg-red-50 dark:bg-red-500/10 px-3 py-1 rounded-full border border-red-100 dark:border-red-500/20 animate-pulse"
              >
                {{ activeFailures().length }}
                {{ activeFailures().length === 1 ? 'unidad' : 'unidades' }} en atención
              </span>
            }
          </div>
          ...
          <div class="flex-1 overflow-y-auto custom-scroll relative">
            <table class="w-full text-left text-sm">
              <thead
                class="bg-slate-50 dark:bg-slate-800 text-slate-500 font-bold text-xs uppercase sticky top-0 z-10 shadow-sm"
              >
                <tr>
                  ...
                  <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                    ...
                    <td class="p-4 pl-6">
                      <div class="flex items-center gap-3">
                        <!-- Priority Badge -->
                        <div
                          [class]="'w-1.5 h-10 rounded-full ' + getPriorityColor(f.prioridad)"
                        ></div>

                        <div>
                          <div class="font-black text-slate-800 dark:text-white text-base">
                            {{ f.economico }}
                          </div>
                          <span
                            [class]="
                              'text-[10px] font-bold uppercase px-1.5 py-0.5 rounded border ' +
                              getPriorityBadgeClass(f.prioridad)
                            "
                          >
                            {{ f.prioridad }}
                          </span>
                    ...
                    <!-- Failure Description with Tooltip -->
                    <td class="p-4">
                      <div class="text-xs text-slate-500 font-medium">
                        {{ f.fechaIngreso | date: 'dd MMM, HH:mm' }}
                      </div>
                      <div
                        class="text-sm font-bold text-slate-700 dark:text-slate-300 max-w-[220px] truncate cursor-help"
                        [title]="f.falla"
                      >
                        {{ f.falla }}
                      </div>
                    ...
                    <td class="p-4 text-center">
                      <div class="flex flex-col items-center">
                        ...
                        <span
                          [class]="
                            'text-[10px] font-bold uppercase ' + getTimeInShop(f.fechaIngreso).color
                          "
                        >
                    ...
                    <td class="p-4 pr-6 text-right">
                      <button
                        (click)="selectAsset(f.economico)"
                        class="bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-200 hover:text-[#ce1126] hover:border-[#ce1126] px-3 py-1.5 rounded-lg text-xs font-bold transition shadow-sm group-hover:shadow-md"
                      >
                  ...
                  <tr>
                    <td colspan="5" class="py-20 text-center">
                      <div class="flex flex-col items-center justify-center opacity-60">
                        <div
                          class="w-16 h-16 bg-emerald-50 dark:bg-emerald-500/10 rounded-full flex items-center justify-center mb-4 border border-emerald-100"
                        >
                          <i class="fas fa-check text-2xl text-emerald-500"></i>
                        </div>
                        <h3 class="text-slate-800 dark:text-white font-bold text-lg">
                          Sin unidades en taller
                        </h3>
                        <p class="text-slate-500 text-sm mt-1 mb-4">
                          La flota opera al 100% de capacidad.
                        </p>
                      </div>
                    </td>
                  </tr>
        ...
        <div class="flex flex-col gap-6">
          <!-- Pareto Cards -->
          <div
            class="bg-white dark:bg-[#1e293b] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/60 p-5"
          >
            <div class="flex justify-between items-center mb-4">
              <h3 class="font-bold text-slate-800 dark:text-white text-sm">Top 3 Problemas</h3>
              <span
                class="text-[10px] font-bold text-slate-400 uppercase bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded"
            ...
            <div class="space-y-3">
              <!-- Item 1 -->
              <button
                (click)="filterByIssue('Hidráulico')"
                class="w-full text-left p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 group hover:border-red-200 transition-colors cursor-pointer relative"
              >
                <div class="flex justify-between items-start mb-2">
                  <span
                    class="text-xs font-bold text-slate-700 dark:text-slate-200 group-hover:text-[#ce1126] transition-colors"
                    >Sistema Hidráulico</span
                  >
                  <span class="text-xs font-black text-red-500">45%</span>
                </div>
                <div
                  class="w-full bg-slate-200 dark:bg-slate-700 h-1.5 rounded-full overflow-hidden mb-3"
                >
                  <div class="bg-red-500 h-full w-[45%]"></div>
                </div>
                <div class="flex justify-between items-center">
                  <span class="text-[10px] text-slate-400 font-medium"
                    >12 incidentes reportados</span
                  >
                  <span
                    class="text-[10px] bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 px-3 py-1.5 rounded-lg text-slate-600 group-hover:text-[#ce1126] group-hover:border-red-100 font-bold shadow-sm transition-all group-hover:shadow-md flex items-center gap-1"
                  >
              ...
              <!-- Item 2 -->
              <button
                (click)="filterByIssue('Frenos')"
                class="w-full text-left p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 group hover:border-amber-200 transition-colors cursor-pointer relative"
              >
                <div class="flex justify-between items-start mb-2">
                  <span
                    class="text-xs font-bold text-slate-700 dark:text-slate-200 group-hover:text-amber-600 transition-colors"
                    >Frenos / Desgaste</span
                  >
                ...
                <div class="flex justify-between items-center">
                  <span class="text-[10px] text-slate-400 font-medium"
                    >8 incidentes reportados</span
                  >
                  <span
                    class="text-[10px] bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 px-3 py-1.5 rounded-lg text-slate-600 group-hover:text-amber-600 group-hover:border-amber-100 font-bold shadow-sm transition-all group-hover:shadow-md flex items-center gap-1"
                  >
                    Ver equipos <i class="fas fa-arrow-right text-[9px]"></i>
                  </span>
                </div>
          ...
          <div
            class="flex-1 bg-white dark:bg-[#1e293b] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/60 p-5 flex flex-col"
          >
            <div class="flex justify-between items-center mb-4">
            ...
            <div class="relative pl-4 border-l border-slate-200 dark:border-slate-700 space-y-6">
              @for (event of recentEvents(); track $index) {
                <div class="relative group">
                  ...
                  <div
                    class="absolute -left-[25px] top-0 h-7 w-7 rounded-full border-2 border-white dark:border-slate-900 flex items-center justify-center text-[10px] text-white shadow-sm transition-transform group-hover:scale-110"
                    [ngClass]="event.color"
                  >
                    <i [class]="'fas ' + event.icon"></i>
                  </div>

                  <p class="text-xs font-bold text-slate-700 dark:text-slate-300 leading-tight">
                    {{ event.title }}
                  </p>
                  <div class="flex items-center gap-2 mt-1">
                    <span class="text-[10px] text-slate-400 font-mono">{{ event.time }}</span>
                    <span
                      class="text-[10px] bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-slate-500 font-bold uppercase"
                      >{{ event.user }}</span
                    >
                  </div>
                </div>
...
export class DashboardComponent {
  ...
  activeFailures = computed(() =>
    this.dataService.forkliftFailures().filter(f => f.estatus !== 'Cerrada')
  );
  operativeCount = computed(
    () => this.dataService.assets().filter(a => a.status.name === 'Operativo').length
  );
  ...
  availabilityGap = computed(() => {
    const p = this.fleetAvailability().percentage;
    const diff = p - 95; // Meta 95
    const sign = diff > 0 ? '+' : '';
    return `Brecha: ${sign}${diff.toFixed(0)} pts`; // Fixed: "pts" instead of "%"
  });
  ...
  formattedCost = computed(() => {
    ...
    // Fixed: Consistent casing "Sin gastos"
  ...
  budgetVariance = computed(() => {
    const cost = this.kpi().totalCostMonth;
    const budget = this.kpi().budgetMonth;
    // Fixed: Avoid -100% variance display on zero cost
    if (cost === 0) return 'Disponible: 100%';
    const variance = ((cost - budget) / budget) * 100;
    return variance > 0
      ? `+${variance.toFixed(1)}% vs Presupuesto`
      : `${variance.toFixed(1)}% vs Presupuesto`;
  });

  // Timeline Mock with Icons
  recentEvents = computed(() => {
    return [
      {
        title: 'Unidad 35526 ingresó a Taller',
        time: 'Hace 15 min',
        user: 'Op. Móvil',
        color: 'bg-red-500',
      ...
      {
        title: 'Refacción "Kit Sellos" solicitada',
        time: 'Hace 45 min',
        user: 'Toyota Tech',
        color: 'bg-blue-500',
        icon: 'fa-box-open'
      ...
      {
        title: 'Turno Matutino inició operaciones',
        time: '06:00 AM',
        user: 'Sistema',
