  ...
  template: `
    @if (asset()) {
      <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden animate-fade-in relative">
        ...
        <!-- Critical Badge -->
        @if (asset()!.critical) {
           <div class="absolute top-0 right-0 bg-yellow-400 text-black text-[10px] uppercase font-black px-4 py-2 rounded-bl-lg z-10 shadow-sm tracking-wide">
             <i class="fas fa-exclamation-triangle mr-1"></i> Prioridad Alta
           </div>
        }

        <!-- Header (Coke Red) -->
        <div class="bg-[#ce1126] text-white p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <div class="flex items-center gap-3">
              <h2 class="text-3xl font-black tracking-tight">{{ asset()!.id }}</h2>
              <span class="px-3 py-0.5 rounded-full text-xs font-bold uppercase bg-white/20 border border-white/30 backdrop-blur-sm">
                {{ asset()!.status.name }}
              </span>
            </div>
          ...
          <div class="flex flex-wrap gap-2">
            <button (click)="close()" class="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-white text-sm font-bold transition border border-white/10" title="Volver">
              <i class="fas fa-arrow-left"></i>
            </button>
            <button (click)="exportPdf()" class="px-4 py-2 rounded-lg bg-white text-[#ce1126] hover:bg-gray-100 text-sm font-bold transition shadow-md flex items-center">
        ...
        <div class="p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div class="grid grid-cols-3 gap-4">
               <div class="p-4 bg-gray-50 rounded-lg border border-gray-100 text-center">
                 <div class="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Combustible</div>
                 <div class="font-bold text-gray-800 text-lg">{{ asset()!.fuelType }}</div>
               </div>
               <div class="p-4 bg-gray-50 rounded-lg border border-gray-100 text-center">
                 <div class="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Adquisición</div>
                 <div class="font-bold text-gray-800 text-lg">{{ asset()!.acquisitionDate | date:'MM/yyyy' }}</div>
               </div>
               <div class="p-4 bg-gray-50 rounded-lg border border-gray-100 text-center">
                 <div class="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Total Fallas</div>
            ...
            <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
               ...
               @if (asset()!.status.name === 'Operativo') {
                  ...
                  <div class="bg-red-50 p-5 rounded-lg border border-red-100 relative">
                    ...
                    <!-- AI Vision Uploader -->
                    <div class="mb-4 p-4 border-2 border-dashed border-red-200 rounded-lg bg-white/50 text-center relative group hover:border-red-400 transition-colors">
                       <input type="file" (change)="handleImageUpload($event)" accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                       <div class="flex flex-col items-center gap-2">
                          ...
                          <p class="text-[10px] text-gray-500">Sube una foto de la avería para autocompletar</p>
                       </div>
                       @if (analyzingImage()) {
                          <div class="absolute inset-0 bg-white/90 z-20 flex items-center justify-center text-[#ce1126] font-bold text-xs gap-2">
                    ...
                    <div class="flex flex-col gap-3">
                      <select #failType class="w-full p-2.5 border border-gray-300 rounded text-sm bg-white focus:ring-2 focus:ring-red-500 outline-none" [value]="inspectionData()?.inspection?.damage_analysis?.damage_type || 'Mecánico'">
                      ...
                      <textarea #failDesc 
                                ...
                                [value]="inspectionData() ? (inspectionData()?.inspection?.asset?.visual_condition + ' - Causa Probable: ' + inspectionData()?.inspection?.root_cause_analysis?.probable_cause) : ''"
                      ></textarea>

                      <!-- AI Analysis Result Preview -->
                      @if (inspectionData()) {
                        <div class="bg-white p-3 rounded border border-red-100 text-xs text-gray-600 mb-2">
                           <p><strong class="text-red-700">Severidad Detectada:</strong> {{ inspectionData()?.inspection?.severity?.level }}</p>
                           <p><strong class="text-red-700">Refacciones Sugeridas:</strong></p>
                           <ul class="list-disc pl-4 mt-1">
                             @for (part of inspectionData()?.inspection?.repair_plan?.estimated_parts; track part.part_name) {
                               <li>{{ part.quantity }}x {{ part.part_name }} ({{ part.generic_code }})</li>
                             }
                           </ul>
                        </div>
                      }
                  ...
                  <div class="bg-green-50 p-5 rounded-lg border border-green-100">
                    ...
                    <div class="flex flex-col gap-3">
                      <textarea #diagnosis class="w-full p-2.5 border border-green-200 rounded text-sm bg-white focus:ring-2 focus:ring-green-500 outline-none" rows="2" placeholder="Diagnóstico final y solución..."></textarea>
                      <div class="grid grid-cols-2 gap-3">
                        <input #cost type="number" class="p-2.5 border border-green-200 rounded text-sm bg-white" placeholder="Costo Refacciones ($)">
                        <input #parts type="text" class="p-2.5 border border-green-200 rounded text-sm bg-white" placeholder="Refacciones (separar por coma)">
                      </div>
                    ...
                    @for (item of history(); track item.id) {
                      <tr class="hover:bg-gray-50 transition"
                        ...
                        <td class="p-3 text-gray-600 align-top whitespace-nowrap font-medium">
                          ...
                          @if (!item.exitDate) {
                             <span class="inline-block px-2 py-0.5 rounded bg-[#ce1126] text-white text-[10px] font-bold mt-1 ml-2">
                              ABIERTO
                            </span>
                          }
                        </td>
                        ...
                        <td class="p-3 align-top">
                          <span class="px-2 py-1 bg-gray-100 text-gray-600 text-[10px] font-bold uppercase rounded whitespace-nowrap">{{ item.type }}</span>
                        </td>
                        <td class="p-3 text-right font-mono text-gray-900 font-bold align-top whitespace-nowrap">{{ item.estimatedCost | currency }}</td>
                      </tr>
                    }
                  </tbody>
                </table>
          ...
          <!-- Column 3: AI Sidebar -->
          <div class="space-y-6">
            <div class="bg-gradient-to-br from-gray-900 to-black p-6 rounded-xl border border-gray-800 text-white shadow-xl">
              <div class="flex items-center gap-3 mb-4">
              ...
              <p class="text-sm text-gray-400 mb-6 leading-relaxed">
                Gemini AI analiza patrones de desgaste, MTBF y condiciones de uso para predecir fallas futuras.
              </p>
              ...
              @if (aiLoading()) {
                ...
                <button (click)="runAnalysis()" class="mt-4 w-full py-2 text-xs font-bold text-gray-400 hover:text-white uppercase tracking-wide border border-gray-700 rounded hover:bg-gray-800 transition">
                  Regenerar Análisis
                </button>
              } @else {
            ...
            @if (lotoResult()) {
               <div class="bg-yellow-50 p-6 rounded-xl border border-yellow-200 shadow-lg">
                  <div class="flex justify-between items-center mb-4">
                     <h3 class="font-black text-yellow-800 flex items-center gap-2"><i class="fas fa-lock"></i> Procedimiento LOTO</h3>
                     <button (click)="lotoResult.set(null)" class="text-yellow-600 hover:text-black"><i class="fas fa-times"></i></button>
                  </div>
                  <div class="prose prose-sm prose-yellow max-h-64 overflow-y-auto custom-scroll" [innerHTML]="lotoResult()"></div>
...
export class AssetDetailComponent {
  ...
  async generateLoto() {
    ...
    this.lotoResult.set(this.sanitizer.sanitize(1, '<p class="text-gray-500 animate-pulse">Generando procedimiento de seguridad...</p>'));
    const html = await this.geminiService.generateLotoProcedure(currentAsset, lastFailure);
    this.lotoResult.set(this.sanitizer.sanitize(1, html));
  }
  ...
  async runAnalysis() {
    ...
    this.aiResult.set(this.sanitizer.sanitize(1, result));
