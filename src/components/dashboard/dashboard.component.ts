      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
         <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 flex items-center gap-2">
               ...
               <i class="far fa-clock text-slate-400"></i> <span class="text-xs">Actualizado: {{ lastUpdate() | date:'HH:mm:ss' }}</span>
            </p>
         </div>
        ...
        <div class="lg:col-span-2 bg-white dark:bg-[#1e293b] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/60 flex flex-col overflow-hidden h-[500px]">
          <div class="p-5 border-b border-slate-100 dark:border-slate-700/50 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/20">
            ...
            @if (activeFailures().length > 0) {
               <span class="text-xs font-bold text-red-600 bg-red-50 dark:bg-red-500/10 px-3 py-1 rounded-full border border-red-100 dark:border-red-500/20 animate-pulse">
                  ...
                  @for (f of activeFailures(); track f.id) {
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                      ...
                      <td class="p-4 pl-6">
                         <div class="flex items-center gap-3">
                            ...
                            <div>
                               ...
                               <span [class]="'text-[10px] font-bold uppercase px-1.5 py-0.5 rounded border ' + getPriorityBadgeClass(f.prioridad)">
                                  {{ f.prioridad }}
                               </span>
                            </div>
                         </div>
                      ...
                      <td class="p-4">
                         ...
                         <div class="text-sm font-bold text-slate-700 dark:text-slate-300 max-w-[220px] truncate cursor-help" [title]="f.falla">
                            {{ f.falla }}
                         </div>
                      </td>
                      ...
                      <td class="p-4 text-center">
                         <div class="flex flex-col items-center">
                            ...
                            <span [class]="'text-[10px] font-bold uppercase ' + getTimeInShop(f.fechaIngreso).color">
                      ...
                      <td class="p-4 pr-6 text-right">
                        <button (click)="selectAsset(f.economico)" class="bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-200 hover:text-[#ce1126] hover:border-[#ce1126] px-3 py-1.5 rounded-lg text-xs font-bold transition shadow-sm group-hover:shadow-md">
                    ...
                    <tr>
                      <td colspan="5" class="py-20 text-center">
                        <div class="flex flex-col items-center justify-center opacity-60">
                           <div class="w-16 h-16 bg-emerald-50 dark:bg-emerald-500/10 rounded-full flex items-center justify-center mb-4 border border-emerald-100">
                           ...
                           <p class="text-slate-500 text-sm mt-1 mb-4">La flota opera al 100% de capacidad.</p>
                        </div>
                      </td>
                    </tr>
                  }
              ...
              <div class="relative pl-4 border-l border-slate-200 dark:border-slate-700 space-y-6">
                 @for (event of recentEvents(); track $index) {
                    <div class="relative group">
                       ...
                       <div class="absolute -left-[25px] top-0 h-7 w-7 rounded-full border-2 border-white dark:border-slate-900 flex items-center justify-center text-[10px] text-white shadow-sm transition-transform group-hover:scale-110" [ngClass]="event.color">
                       ...
                       <div class="flex items-center gap-2 mt-1">
                          ...
                          <span class="text-[10px] bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-slate-500 font-bold uppercase">{{ event.user }}</span>
                       </div>
                    </div>
                 }
              </div>
...
export class DashboardComponent {
  ...
  getTimeInShop(dateStr: string) {
    ...
    if (diffHrs < 1) {
       text = '< 1h'; label = 'A tiempo'; color = 'text-emerald-500';
    } else if (diffHrs < 24) {
       text = `${diffHrs}h`; label = 'En proceso'; color = 'text-blue-500';
    } else {
       const days = Math.floor(diffHrs / 24);
       text = `${days}d ${diffHrs % 24}h`; label = 'Demorado'; color = 'text-amber-500';
    }
